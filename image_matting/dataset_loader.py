...
class BatchDatset:
    ...
    def __init__(self, imgs_path, batch_size=5):
        ...
        self.ori_imgs, self.cur_imgs, self.cur_labels = self.get_variations(self.imgs[0])

    def next_batch(self):
        while len(self.cur_imgs) < self.batch_size: # if not enough, get the next image
            self.cur_ind += 1
            if self.cur_ind >= len(self.imgs):      #have complated epochs
                self.epochs_completed += 1
                print("*************** Epochs completed: " + str(self.epochs_completed) + "****************")
                self.cur_ind = 0
            
            cur_name = self.imgs[self.cur_ind]
            tmp_ori_imgs, tmp_imgs, tmp_labels = self.get_variations(cur_name)
            self.ori_imgs += tmp_ori_imgs
            self.cur_imgs += tmp_imgs
            self.cur_labels += tmp_labels
        if len(self.cur_imgs) >= self.batch_size:
            romat = np.zeros((self.batch_size, self.img_height, self.img_width, 3), dtype=np.float)
    ...
    def get_variations(self, img_name):
        ...
        if img_name < 10:
            ...
            stp = '0' + stp
        img_path = 'data/fcn_vgg/portraitFCN_data/' + stp + '.mat'
        alpha_path = 'data/fcn_vgg/images_mask/' + stp + '_mask.mat'
        if os.path.exists(img_path) and os.path.exists(alpha_path):
            imat = sio.loadmat(img_path)['img']             #normal image
            ...
            ori_image.append(org_mat)

            i_img = Image.fromarray(np.uint8(org_mat))          #origin
            a_img = Image.fromarray(np.uint8(amat))             #label
            ...
            for i in range(4):
                ...
                ori_image.append(tmpri_img)
                imgs.append(rimat)              #again normal
                labels.append(np.array(tmpa_img, dtype=np.int))

                #change RGB
                tmp_ori_nimat = np.array(imat, dtype=np.float)
...
class TestDataset:
    ...
    def next_batch(self):
        ...
        while len(cur_imgs) < self.batch_size: # if not enough, get the next image
            ...
            if self.cur_ind >= len(self.imgs):
                break
            cur_name = self.imgs[self.cur_ind]
            tmg_img, tmp_org = self.get_images(cur_name)
            if tmg_img is not None:
                cur_imgs.append(tmg_img)
                cur_orgs.append(tmp_org)
        if len(cur_imgs) == self.batch_size:
            #print('getting', self.cur_ind)
            rimat = np.zeros((self.batch_size, self.img_height, self.img_width, 3), dtype=np.float)
    ...
    def get_images(self, img_name):
        ...
        if img_name < 10:
            ...
            stp = '0' + stp
        img_path = 'data/test/portraitFCN_data/' + stp + '.mat'
